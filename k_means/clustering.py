...
class Point:
    ...
    def euclidean_distance(self, another_point):
        return math.sqrt(math.pow(
                self.latit - another_point.latit, 2.0) + math.pow(self.longit - another_point.longit, 2.0))
...
class KMeans:
        ...
        for point_1 in points:
            ...
            for cluster in clusters.values():
                ...
                if point_1 not in dist:
                    dist[point_1] = math.sqrt(math.pow(
                        point_1.latit - point_2.latit, 2.0) + math.pow(point_1.longit - point_2.longit, 2.0))
                else:
                    dist[point_1] += math.sqrt(math.pow(point_1.latit - point_2.latit, 2.0) + math.pow(
                        point_1.longit - point_2.longit, 2.0))
        if self.debug:
    ...
    def initial_means(self, points):
        """compute the initial means. pick the first node at random."""
    ...
    def assign_points(self, points):
        ...
        for point in points:
            ...
            for mean in self.means:
                dist.append(math.sqrt(math.pow(point.latit - mean.latit,
                            2.0) + math.pow(point.longit - mean.longit, 2.0)))
    ...
    def update_means(self, means, threshold):
        ...
        for i in range(len(self.means)):
            ...
            if math.sqrt(math.pow(
                    mean_1.latit - mean_2.latit, 2.0) + math.pow(mean_1.longit - mean_2.longit, 2.0)) > threshold:
                return False
        return True
